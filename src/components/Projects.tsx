...
// Yeh project ab upar Highlight (bade card) mein dikhega
  ...
  return (
    <section id="projects" className="py-24 bg-zinc-50 dark:bg-zinc-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* --- HIGHLIGHTED PROJECT (School Management) --- */}
        <motion.div 
        ...
        {/* --- FEATURE PROJECTS GRID (REST API & E-commerce) --- */}
        <div className="text-center mb-12">
          <h2 className="text-2xl font-bold text-zinc-900 dark:text-white">Feature Projects</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {featureProjects.map((project, index) => (
            <motion.div
              ...
              className="bg-white dark:bg-zinc-900 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 hover:shadow-lg transition-all flex flex-col h-full"
            >
              <div className="p-6 flex-grow">
                ...
                <div className="flex flex-wrap gap-2">
                  {project.tech.map((tech, i) => (
                    <span key={i} className="px-3 py-1 text-xs font-medium bg-zinc-100 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-300 rounded-full">
                      {tech}
                    </span>
                  ))}
                </div>
              ...
              <div className="px-6 py-4 bg-zinc-50 dark:bg-zinc-800/50 border-t border-zinc-100 dark:border-zinc-800 flex justify-between items-center">
                <a href={project.github} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium text-zinc-600 hover:text-emerald-600 dark:text-zinc-400 dark:hover:text-emerald-400 transition-colors">
                  <Github className="w-4 h-4 mr-2" /> Code
                </a>
                <a href={project.demo} target="_blank" rel="noopener noreferrer" className="flex items-center text-sm font-medium text-zinc-600 hover:text-emerald-600 dark:text-zinc-400 dark:hover:text-emerald-400 transition-colors">
